...
public class ExtendedGpsColorNearInfraredPointTests
{
    ...
    [
        0xDE, 0x35, 0x5B, 0x00, 0xD4, 0x9B, 0x0E, 0x00, 0x87, 0x2B, 0xF8, 0xFF, 0xB6, 0x8C, 0x21, 0x00, 0x04, 0x00, 0x72, 0x78, 0x00, 0x00, 0x47, 0x87, 0x67, 0x62, 0x97, 0xA7, 0xA5, 0x41, 0x67, 0x62, 0x97, 0xA7, 0xA5, 0x41, 0xA5, 0x41,
    ];

    private static readonly ExtendedGpsColorNearInfraredPointDataRecord Point = new()
    {
        X = 5977566,
        Y = 957396,
        Z = -513145,
    ...
    {
        await CheckPoint(MemoryMarshal.Read<ExtendedGpsColorNearInfraredPointDataRecord>(Bytes));
    }
    ...
    private static async Task CheckPoint(ExtendedGpsColorNearInfraredPointDataRecord record)
    {
        await Assert.That(record)
            ...
            .Satisfies(p => p.NearInfrared, nearInfrared => nearInfrared.IsNotZero());
    }
}