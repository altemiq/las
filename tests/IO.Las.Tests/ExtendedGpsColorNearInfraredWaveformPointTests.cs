...
public class ExtendedGpsColorNearInfraredWaveformPointTests
{
    ...
    private static readonly ExtendedGpsColorNearInfraredWaveformPointDataRecord Point = new()
    {
        X = 5977566,
        Y = 957396,
        Z = -513145,
    ...
    private static async Task CheckPoint(ExtendedGpsColorNearInfraredWaveformPointDataRecord record)
    {
        await Assert.That(record)
            ...
            .Satisfies(
                p => p.Color,
                color => color
                    .Satisfies(c => c.R, r => r.IsEqualTo((ushort)25191))
                    .Satisfies(c => c.G, g => g.IsEqualTo((ushort)42903))
                    .Satisfies(c => c.B, b => b.IsEqualTo((ushort)16805)))
