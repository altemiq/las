...
public class AmazonS3UriExtensionsTests
{
    ...
    {
        await Assert.That(new Amazon.S3.Util.AmazonS3Uri(Data.S3UriData.S3Scheme).ToS3Style()).IsEqualTo(new(Data.S3UriData.S3Scheme));
    }
    ...
    {
        await Assert.That(new Amazon.S3.Util.AmazonS3Uri(input).ToPathStyle()).IsEqualTo(new(expected));
    }

    [Test]
    public async Task ToVirtualHostedStyle()
    {
        await Assert.That(new Amazon.S3.Util.AmazonS3Uri(Data.S3UriData.PathStyle).ToVirtualHostStyle()).IsEqualTo(new(Data.S3UriData.VirtualHostedStyle));
    }
