...
public class LasZipTests
{
    ...
    {
        _ = await Assert.That(static () => new LasZip(PointDataRecord.Id, 0, (Compressor)ushort.MaxValue).Validate(PointDataRecord.Size)).Throws<InvalidDataException>();
    }
#else
    [Test]
    public async Task InvalidCompressorValue()
    {
        _ = await Assert.That(static () => new LasZip(PointDataRecord.Id, (Compressor)ushort.MaxValue).Validate(PointDataRecord.Size)).Throws<InvalidDataException>();
    }
#endif
}