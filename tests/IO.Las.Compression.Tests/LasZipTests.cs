...
public class LasZipTests
{
    ...
    {
        await Assert.That(() => _ = new LasZip(ExtendedGpsPointDataRecord.Id, 0, compressor, LasZip.GetValidVersion(ExtendedGpsPointDataRecord.Id, HeaderBlock.DefaultVersion))).Throws<ArgumentException>();
    }
    ...
    public async Task AddExtraBytes(byte pointDataFormatId, LasItemType lasItemType)
    {
        ...
        await Assert.That(lasZip.Items).Contains(new LasItem { Type = lasItemType, Version = version, Size = 2 });
    }

    [Test]
    public async Task InvalidCompressorValue()
    {
        await Assert.That(() => new LasZip(PointDataRecord.Id, 0, (Compressor)ushort.MaxValue).Validate(PointDataRecord.Size)).Throws<InvalidDataException>();
    }
#else
    [Test]
    public async Task InvalidCompressorValue()
    {
        await Assert.That(() => new LasZip(PointDataRecord.Id, (Compressor)ushort.MaxValue).Validate(PointDataRecord.Size)).Throws<InvalidDataException>();
    }
#endif
}