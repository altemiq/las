...
public class CopcExtensionsTests
{
    ...
    public async Task ToBoundingBox()
    {
        ...
        _ = await Assert.That(key.ToBoundingBox(min, max))
    ...
    {
        _ = await Assert.That(key.Parent()).IsEqualTo(expected);
    }

    public IEnumerable<Func<(CopcHierarchy.VoxelKey Key, CopcHierarchy.VoxelKey Expected)>> GetParentData()
    {
        yield return static () => (new(1, 0, 1, 1), new(0, 0, 0, 0));
        yield return static () => (new(1, 2, 1, 1), new(0, 1, 0, 0));
        yield return static () => (new(1, 1, 2, 1), new(0, 0, 1, 0));
        yield return static () => (new(1, 1, 1, 2), new(0, 0, 0, 1));
    }
}