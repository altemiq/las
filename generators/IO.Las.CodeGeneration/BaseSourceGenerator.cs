...
public abstract class BaseSourceGenerator : IIncrementalGenerator
{
    ...
    protected static BaseNamespaceDeclarationSyntax? GetNamespace(string? @namespace) => @namespace is not null ? FileScopedNamespaceDeclaration(ParseName(@namespace)) : null;
    ...
    /// <summary>
    /// Gets whether we have code coverage attributes.
    /// </summary>
    /// <param name="context">The context.</param>
    /// <returns>The incremental value provider.</returns>
    protected static IncrementalValueProvider<bool> GetExcludeFromCodeCoverage(IncrementalGeneratorInitializationContext context) => context.CompilationProvider.Select(static (compilation, _) => compilation.GetTypeByMetadataName("System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverageAttribute") is not null);
    ...
    protected static CompilationUnitSyntax CreateEnum(BaseNamespaceDeclarationSyntax? @namespace, string name, string description, IReadOnlyCollection<string?> ids, bool excludeFromCodeCoverage)
    {
                ...
                .WithMembers(
                    SingletonList<MemberDeclarationSyntax>(
                        @namespace
                            ...
                            .WithMembers(
                                List(
                                    GetMembers(name, description, ids, excludeFromCodeCoverage)))));
        }
        ...
        static IEnumerable<MemberDeclarationSyntax> GetMembers(string name, string description, IReadOnlyCollection<string?> ids, bool excludeFromCodeCoverage)
        {
            yield return EnumDeclaration(name)
                .WithModifiers(
                    TokenList(
                        Token(
                            TriviaList(
            ...
            yield return ClassDeclaration("ExtensionMethods")
                .WithModifiers(
                    TokenList(
                        Token(
                            TriviaList(
    ...
    /// <summary>
    /// Gets the identifier for the type declaration.
    /// </summary>
    /// <param name="declarationSyntax">The declaration.</param>
    /// <returns>The identifier.</returns>
    protected static string GetIdentifier(BaseTypeDeclarationSyntax declarationSyntax) => declarationSyntax.Identifier.ToString();

    /// <summary>
    /// Gets the generator type.
    /// </summary>
    /// <param name="context">The context.</param>
    /// <returns>The type to generate.</returns>
    protected static TypeToGenerate GetTypeToGenerate(GeneratorSyntaxContext context)
    {
        if (context.Node is not BaseTypeDeclarationSyntax declarationSyntax)
        {
            return default;
        }

        var name = GetIdentifier(declarationSyntax);
        var @namespace = GetNamespaceDeclaration(declarationSyntax) is { Name: var namespaceName }
            ? namespaceName.ToString()
            : null;

        return new(@namespace, name);

        static BaseNamespaceDeclarationSyntax? GetNamespaceDeclaration(SyntaxNode node)
        {
