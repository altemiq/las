...
public sealed class ModelSourceGenerator : BaseSourceGenerator
{
    /// <inheritdoc/>
    public override void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var modelCsvArray = modelCsv.Collect();
        var modelsToGenerate = model
            .Combine(modelCsvArray)
            .Combine(GetExcludeFromCodeCoverage(context));

        context.RegisterSourceOutput(modelsToGenerate, static (context, modelToGenerate) =>
        {
            var modelCsv = modelToGenerate.Left.Right.Single();
            var models = GetModels(modelCsv);
            var @namespace = GetNamespace(modelToGenerate.Left.Left.Namespace);
            var excludeFromCodeCoverage = modelToGenerate.Right;

            context.AddSource("Brand.g.cs", GetSourceText(CreateBrands(@namespace, GetBrands(modelCsv), excludeFromCodeCoverage)));
            context.AddSource("Models.g.cs", GetSourceText(CreateModels(@namespace, models)));
            context.AddSource("Model.g.cs", GetSourceText(CreateModelsParse(@namespace, models, excludeFromCodeCoverage)));
        });
    ...
    private static CompilationUnitSyntax CreateModelsParse(BaseNamespaceDeclarationSyntax? @namespace, IReadOnlyCollection<(string? Brand, string? Model, string? Code)> models, bool excludeFromCodeCoverage)
    {
                ...
                .WithMembers(
                    SingletonList<MemberDeclarationSyntax>(
                        @namespace
                            ...
                            .WithMembers(
                                SingletonList(
                                    CreateRecord(models, excludeFromCodeCoverage)))));
        }
        ...
        static MemberDeclarationSyntax CreateRecord(IReadOnlyCollection<(string? Brand, string? Model, string? Code)> models, bool excludeFromCodeCoverage)
        {
